...
if ( !load_saved_LCD ) {
  theta_XY <- cbind( -swissRain_Stan_data_real_plot$Theta_Y,
                     swissRain_Stan_data_real_plot$Theta_X )
  ...
  saveRDS( theta_XY_lcd, file = "Results/swissRain_Polar_LCD.RDS" )
}


### Load saved density estimates (load_saved_LCD <- TRUE)
if ( load_saved_LCD ) { 
  theta_XY_lcd <- readRDS( "Results/swissRain_Polar_LCD.RDS" )  
}

### Obtain grid points
N = 100
h_theta <- c( MASS::bandwidth.nrd( -swissRain_Stan_data_real_plot$Theta_Y ),
              MASS::bandwidth.nrd( swissRain_Stan_data_real_plot$Theta_X ) )
real_grid_points_theta <- MASS::kde2d( -swissRain_Stan_data_real_plot$Theta_Y,
                                       swissRain_Stan_data_real_plot$Theta_X,
                                       h = h_theta, n = N )
...
png( file = "Figures/swissRain_Polar_LC2D.png",
  ...
  hdrcde:::hdrcde.filled.contour(
    ...
    xlim = c(0.5,5), ylim = c(-1.5,3.0), 
    plot.axes = { title( main = "", 
                         xlab = expression( theta[1] ), 
                         ylab = expression( theta[2] ),
                         cex.lab = 1.80 )
      axis(1, seq(from = 0.5, to = 5.5, by = 1.0), cex.axis = 1.40) 
      axis(2, seq(from = -1.0, to = 3.0, by = 1.0), cex.axis = 1.40) } )
  if ( include_legend ) { 
    legend( x = 3.5, y = -0.25, bg = "white",
...
### Obtain grid points
N = 100
h_phi <- c( MASS::bandwidth.nrd( swissRain_Stan_data_real_plot$Angle ),
            MASS::bandwidth.nrd( swissRain_Stan_data_real_plot$Radius ) )
real_grid_points_phi <- MASS::kde2d( swissRain_Stan_data_real_plot$Angle,
                                     swissRain_Stan_data_real_plot$Radius,
                                     h = h_phi, n = N )
...
png( file = "Figures/swissRain_Angle_Ratio_LC2D.png",
     res = 300, width = 1.25 * 1928, height = 1.25 * 1514, units = "px" )
  ...
  hdrcde:::hdrcde.filled.contour(
    ...
    color.palette = function(n) hcl.colors(n, "YlOrRd", rev = TRUE),
    xlim = c(1,26), ylim = c(15,55), 
    plot.axes = { title( main = "", 
                         xlab = expression( rho ), 
                         ylab = expression( psi~~"(Degrees)" ),
                         cex.lab = 1.80 )
      axis(1, c(1,5,10,15,20,25), cex.axis = 1.40) 
      axis(2, seq(from = 15, to = 55, by = 10), cex.axis = 1.40) } )
  if ( include_legend ) { 
    legend( x = 17, y = 27, bg = "white",
            legend = rev( scales::percent(
              c(0.25, 0.50, 0.80, 0.90, 0.95, 0.99),
              accuracy = 1 ) ),
            fill = rev( hcl.colors(6, "YlOrRd") ),
...
### Perform density estimation (load_saved_LCD <- FALSE)
if ( !load_saved_LCD ) { 
  theta_XY <- cbind( swissRain_stanfit$theta_1,
                     swissRain_stanfit$theta_2 )
  theta_XY_rounded <- round( theta_XY, digits = LCD_digits )
  theta_XY_weights <- getweights( theta_XY_rounded )
  theta_XY_lcd <- mlelcd(x = theta_XY_weights$x, w = theta_XY_weights$w)
  saveRDS( theta_XY_lcd, file = "Results/swissRain_TransIso_LCD.RDS" )
}

### Load saved density estimates (load_saved_LCD <- TRUE)
if ( load_saved_LCD ) { 
  theta_XY_lcd <- readRDS( file = "Results/swissRain_TransIso_LCD.RDS" )   
}


### Obtain grid points
N = 100
h_theta <- c( MASS::bandwidth.nrd( swissRain_stanfit$theta_1 ),
              MASS::bandwidth.nrd( swissRain_stanfit$theta_2 ) )
real_grid_points_theta <- MASS::kde2d( swissRain_stanfit$theta_1,
                                        swissRain_stanfit$theta_2,
                                        h = h_theta, n = N )
...
png( file = "Figures/swissRain_TransIso_LC2D.png",
     res = 300, width = 1.25 * 1928, height = 1.25 * 1514, units = "px" )
  ...
  hdrcde:::hdrcde.filled.contour(
    ...
    plot.axes = { title( main = "", 
      ...
      axis(2, seq(from = -7.5, to = -4.5, by = 1.0), cex.axis = 1.40) } )
  if ( include_legend ) {
...
### Obtain grid points
N = 100
h_phi <- c( MASS::bandwidth.nrd( swissRain_stanfit$phi_1 ),
            MASS::bandwidth.nrd( swissRain_stanfit$phi_2 ) )
real_grid_points_phi <- MASS::kde2d( swissRain_stanfit$phi_1, # sigma 
                                      swissRain_stanfit$phi_2, # gamma
                                      h = h_phi, n = N )
...
for (ii in 1:N) {
  for (jj in 1:N) {
    ...
    # theta_3
    x2$x[ii,jj] <- log( real_grid_points_phi$x[ii] ) / sqrt(2) + 
                   log( real_grid_points_phi$y[jj] ) / sqrt(2)
    # theta_4
    x2$y[ii,jj] <- log( real_grid_points_phi$x[ii] ) * sqrt(2) - 
                   log( real_grid_points_phi$y[jj] ) * sqrt(2) 
...
png( file = "Figures/swissRain_Sigma_Gamma_LC2D.png",
     res = 300, width = 1.25 * 1928, height = 1.25 * 1514, units = "px" )

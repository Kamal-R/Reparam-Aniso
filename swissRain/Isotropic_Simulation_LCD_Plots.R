...
# TRUE: Load saved density estimates (if the script has been run before)
...
if ( !load_saved_LCD ) {
  theta_XY <- cbind( -swissRain_Stan_data_iso_plot$Theta_Y,
                     swissRain_Stan_data_iso_plot$Theta_X )
  ...
  saveRDS( theta_XY_lcd, file = "Results/Iso_Sim_Polar_LCD.RDS" )
}

### Load saved density estimates (load_saved_LCD == TRUE)
if ( load_saved_LCD ) { 
  theta_XY_lcd <- readRDS( file = "Results/Iso_Sim_Polar_LCD.RDS" ) 
}
  
### Obtain grid points
N = 100
h_theta <- c( MASS::bandwidth.nrd( -swissRain_Stan_data_iso_plot$Theta_Y ),
              MASS::bandwidth.nrd( swissRain_Stan_data_iso_plot$Theta_X ) )
iso_grid_points_theta <- MASS::kde2d( -swissRain_Stan_data_iso_plot$Theta_Y,
                                      swissRain_Stan_data_iso_plot$Theta_X,
                                      h = h_theta, n = N )

### Evaluate density at grid points
log_concave_density_theta <- expand.grid("x" = iso_grid_points_theta$x,
                                         "y" = iso_grid_points_theta$y)
...
N = 100
h_phi <- c( MASS::bandwidth.nrd( swissRain_Stan_data_iso_plot$Angle ),
            MASS::bandwidth.nrd( swissRain_Stan_data_iso_plot$Radius ) )
iso_grid_points_phi <- MASS::kde2d( swissRain_Stan_data_iso_plot$Angle,
                                    swissRain_Stan_data_iso_plot$Radius,
                                    h = h_phi, n = N )
...
for (ii in 1:N) {
  for (jj in 1:N) {
    ...
    # theta_1 
    x2$x[ii,jj] <- sqrt( iso_grid_points_phi$y[ii] ) *
      sin( 2 * pi/180 * iso_grid_points_phi$x[jj] ) 
    # theta_2
    x2$y[ii,jj] <- sqrt( iso_grid_points_phi$y[ii] ) *
      cos( 2 * pi/180 * iso_grid_points_phi$x[jj] ) 
...
png( file = "Figures/Iso_Sim_Angle_Ratio_LC2D.png",
     res = 300, width = 1.25 * 1928, height = 1.25 * 1514, units = "px" )
  ...
  hdrcde:::hdrcde.filled.contour(
    ...
    color.palette = function(n) hcl.colors(n, "YlOrRd", rev = TRUE),
    xlim = c(1.0,4.0), ylim = c(-90,90), cex.lab = 1.80,
    plot.axes = { title( main = "", 
                         xlab = expression( rho ), 
                         ylab = expression( psi~~"(Degrees)" ),
                         cex.lab = 1.80 )
      axis(1, seq(from = 0, to = 4.0, by = 0.5), cex.axis = 1.40)
      axis(2, seq(from = -90, to = 90, by = 30), cex.axis = 1.40) } )
  if ( include_legend ) { 
    legend( x = 3.00, y = 40, bg = "white",
            legend = rev( scales::percent(
              c(0.25, 0.50, 0.80, 0.90, 0.95, 0.99),
              accuracy = 1 ) ),
            fill = rev( hcl.colors(6, "YlOrRd") ),
...
### Perform density estimation (load_saved_LCD <- FALSE)
if ( !load_saved_LCD ) {
  theta_XY <- cbind( swissRain_stanfit_iso$theta_1,
                     swissRain_stanfit_iso$theta_2 )
  theta_XY_rounded <- round( theta_XY, digits = LCD_digits )
  theta_XY_weights <- getweights( theta_XY_rounded )
  theta_XY_lcd <- mlelcd(x = theta_XY_weights$x, w = theta_XY_weights$w)
  saveRDS( theta_XY_lcd, file = "Results/Iso_Sim_TransIso_LCD.RDS" )
}

### Load saved density estimates (load_saved_LCD <- TRUE)
if ( load_saved_LCD ) { 
  theta_XY_lcd <- readRDS( "Results/Iso_Sim_TransIso_LCD.RDS" )  
}


### Obtain grid points
N = 100
h_theta <- c( MASS::bandwidth.nrd( swissRain_stanfit_iso$theta_1 ),
              MASS::bandwidth.nrd( swissRain_stanfit_iso$theta_2 ) )
iso_grid_points_theta <- MASS::kde2d( swissRain_stanfit_iso$theta_1,
                                      swissRain_stanfit_iso$theta_2,
                                      h = h_theta, n = N )
...
png( file = "Figures/Iso_Sim_TransIso_LC2D.png",
     res = 300, width = 1.25 * 1928, height = 1.25 * 1514, units = "px" )
  ...
  hdrcde:::hdrcde.filled.contour(
    ...
    levels = c( credible_region_levels_theta, max(theta_XY_theta_eval) ),
    color.palette = function(n) hcl.colors(n, "YlOrRd", rev = TRUE), 
    xlim = c(1.5,5.5), ylim = c(-7.5,-4.0), cex.lab = 1.80,
    plot.axes = { title( main = "", 
                         xlab = expression( theta[3] ), 
                         ylab = expression( theta[4] ),
                         cex.lab = 1.80 )
      axis(1, seq(from = 1.5, to = 5.5, by = 1.0), cex.axis = 1.40)
      axis(2, seq(from = -7.5, to = -3.5, by = 1.0), cex.axis = 1.40) } )
  if ( include_legend ) { 
    legend( x = 4.25, y = -4.25, bg = "white",
            legend = rev( scales::percent(
              c(0.25, 0.50, 0.80, 0.90, 0.95, 0.99),
              accuracy = 1 ) ),
            fill = rev( hcl.colors(6, "YlOrRd") ),
...
### Obtain grid points
N = 100
h_phi <- c( MASS::bandwidth.nrd( swissRain_stanfit_iso$phi_1 ),
            MASS::bandwidth.nrd( swissRain_stanfit_iso$phi_2 ) )
iso_grid_points_phi <- MASS::kde2d( swissRain_stanfit_iso$phi_1, # sigma 
                                    swissRain_stanfit_iso$phi_2, # gamma
                                    h = h_phi, n = N )
...
for (ii in 1:N) {
  for (jj in 1:N) {
    ...
    # theta_3
    x2$x[ii,jj] <- log( iso_grid_points_phi$x[ii] ) / sqrt(2) + 
                   log( iso_grid_points_phi$y[jj] ) / sqrt(2)
    # theta_4
    x2$y[ii,jj] <- log( iso_grid_points_phi$x[ii] ) * sqrt(2) - 
                   log( iso_grid_points_phi$y[jj] ) * sqrt(2)
...
png( file = "Figures/Iso_Sim_Sigma_Gamma_LC2D.png",
     res = 300, width = 1.25 * 1928, height = 1.25 * 1514, units = "px" )
  ...
  hdrcde:::hdrcde.filled.contour(
    ...
    xlim = c(0,3.25), ylim = c(0,200), cex.lab = 1.80,
    plot.axes = { title( main = "", 
